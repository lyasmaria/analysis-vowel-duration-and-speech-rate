...
# on combine deux tableaux pour créer les histogrammes :
combined_data = rbind(data1, data2_balanced)
# histogramme de la distribution selon la durée :
ggplot(combined_data, aes(x = DUREE)) +
  geom_histogram(binwidth = 0.01, color = "black", fill = "lightblue", alpha = 0.7) +
  labs(title = "Distribution de la durée (données combinées)", 
...
#boxplot pour la même tâche :
ggplot(combined_data, aes(y = DUREE)) +
  geom_boxplot(fill = "lightblue", color = "black") +
  labs(title = "Variation de la durée (données combinées)", 
       y = "Durée (en secondes)")

# on transforme la colonne APERTURE pour faire un histogramme :
...
# on fait un histogramme pour voir la distribution de niveaux d'aperture :
...
# scatterplot pour aperture + débit :
...
t_test_aperture <- t.test(DUREE ~ APERTURE, data = combined_data)
